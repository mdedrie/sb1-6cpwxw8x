...
export const FloatingToolbox: React.FC<FloatingToolboxProps> = ({
  ...
  const getAvailableTools = () => {
    switch (context) {
      case 'catalog':
        return [
          { id: 'debug', label: debugTitle, closed: closedPanels.has('debug') },
          { id: 'api', label: apiTitle, closed: closedPanels.has('api') }
        ];
      case 'editor':
        return [
          { id: 'debug', label: debugTitle, closed: closedPanels.has('debug') },
  ...
  return (
    isVisible && <div className="fixed bottom-4 right-4 z-50">
            ...
            <div id="toolbox-content" className="p-4 space-y-2">
              {tools.map(tool => (
                <div key={tool.id} className="flex items-center justify-between">
                  <button
                    onClick={() => tool.closed ? handleReopenPanel(tool.id) : handleToolClick(tool.id as 'debug' | 'api')}
